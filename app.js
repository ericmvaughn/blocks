...
function cb_ready(err, cc) {																	//response has chaincode functions
  if (err) {
    console.log('! looks like an error loading the chaincode or ' +
                'network, app will fail\n', err);
    if (!process.error) {
      process.error = {type: 'load', msg: err.details};				//if it already exist, keep the last error
    }
  }	else {
    chaincode = cc;
    if (!cc.details.deployed_name || cc.details.deployed_name === '') {					//decide if i need to deploy
      ...
      //  way to do this.
      cc.deploy('init', ['a','101','b','200'],
                {save_path: './cc_summaries', delay_ms: 50000}, cb_deployed);
    } else {
      console.log('chaincode summary file indicates chaincode has been ' +
                  'previously deployed');
      cb_deployed();
    }
  }
...
function cb_deployed(e, d) {
  if (e) {
    ...
    console.log('! looks like a deploy error, holding off on ' +
                'starting the socket\n', e);
    if (!process.error) {
...
ibc.chain_stats(function(e, stats) {
  if (e) {
    console.log('error getting chain stats: ', e);
  } else {
...
ibc.monitor_blockheight(function(stats) {
  if (stats === null) {
    ...
    console.log('got the chain stats from monitor_blockheight and ' +
                'the height is: ', stats.height);
    chainHeight = stats.height;
  }
});
